...
test("renders install page markup", () => {
  ...
  assert.match(html, /Add Mon studio KREIA to your/);
...
test("renders the manifest with the per-app name", () => {
  ...
  const fromSite = JSON.parse(
    renderWebManifest("localhost:8080", { name: "Mon studio KREIA", color: "0B0D10" }),
  );
  assert.equal(fromSite.name, "Mon studio KREIA");
  assert.equal(fromSite.short_name, "Mon studio KREIA");
  assert.equal(fromSite.theme_color, "#0B0D10");
});
