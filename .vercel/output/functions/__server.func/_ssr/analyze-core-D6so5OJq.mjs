...
import { B as tryExtractJson, C as fruitHumanoidPromptBlock, D as listCastBatches, I as styleWeave, N as parseProduction, O as lockCharactersSourceNames, R as timedFetchPublic, S as formatLockedDialogue, T as identityParagraph, _ as enforceProductionDialogues, b as fail, c as attachDialogues, d as chat, f as composeCharacterImagePrompt, i as apiKey, j as parseAnalysis, m as duplicateWarnings, n as NETWORK_MESSAGE, o as applyLinesToScenes, r as angelPromptBlock, s as applyNameSubstitutionsToBible, t as INVALID_AI_MESSAGE, v as enforceProductionIdentity, w as identifyCharacters, x as fitDialoguesToScenes, y as extractJson, z as transcribeWav } from "./cast-edit-B0U-aGNG.mjs";
import { n as collapseAnalysisScenes, o as proposeSegments, r as collapseProductionScenes, t as closestPromptDuration } from "./duration-CgP428Xm.mjs";
import { n as progressAt } from "./analysis-stages-DZplH0Sn.mjs";
import { r as formatUserBrief, t as briefCountWarning } from "./user-brief-BwowZZX-.mjs";
import { n as styleFromUserChoice } from "./visual-styles-BkOrGZiu.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/analyze-core-D6so5OJq.js
...
var SCENE_TOKENS = 1800;
function localCharacters(analysis) {
...
function fallbackScene(analysis, index) {
	const scene = analysis.scenes[index];
	const n = scene?.number ?? index + 1;
	const locked = formatLockedDialogue((analysis.dialogues?.lines ?? []).filter((l) => l.sceneNumber === n));
	...
	const spoken = locked ? `Spoken French dialogue, one speaker at a time, others mouths closed: ${locked}` : "No spoken dialogue.";
	return {
		number: n,
		duration: 10,
...
function sceneUserPrompt(input, index, first) {
	const { analysis, mode, durationSeconds, userNotes } = input;
	const scene = analysis.scenes[index];
	const locked = formatLockedDialogue((analysis.dialogues?.lines ?? []).filter((l) => l.sceneNumber === (scene?.number ?? index + 1)));
	const who = scene ? expandCharacterIds(scene.characters, analysis.characters) : "";
...
async function runOneProductionScene(input, checkpoint, index) {
	const analysis = input.analysis;
	const first = index === 0;
	const result = await chat({
		messages: [{
			role: "system",
			content: `${buildGenerationSystemPrompt(input.kind)}\nUne SEULE scène par réponse.`
		}, {
			role: "user",
			content: sceneUserPrompt(input, index, first)
		}],
	...
	if (result && result.ok) try {
		const json = extractJson(result.text);
		const wrapped = json.scene ? {
			...json,
			scenes: [json.scene]
		} : json.scenes ? json : { scenes: [json] };
...
async function runProductionSlice(input) {
	const analysis = input.analysis;
	const checkpoint = {
		version: 1,
	...
	if (index >= total) {
		...
		return {
			checkpoint: {
				...checkpoint,
				production,
				analyzedProductionSceneCount: total
			},
			done: true,
			production,
			progress: progressAt(7, {
				productionScenesDone: total,
	...
	if (!out.ok) return {
		...
		error: out.error || "L'analyse n'a pas pu aboutir. Réessayez.",
		progress: progressAt(7, {
			productionScenesDone: index,
			productionScenesTotal: total
		})
	};
	...
	if (done) {
		...
		if (!checkpoint.completed.includes("produce")) checkpoint.completed = [...checkpoint.completed, "produce"];
	}
	return {
		checkpoint,
		done,
		production: done ? checkpoint.production : void 0,
		progress: progressAt(7, {
...
function sealPlan(plan, analysis, input) {
	let production = plan;
	...
	production = enforceProductionIdentity(enforceProductionDialogues(production, analysis, input.mode, input.kind), analysis);
	return production;
}
var MAX_FRAMES = 12;
...
async function runOneCastBatch(data, frames, checkpoint) {
	...
	checkpoint.limitations = [.../* @__PURE__ */ new Set([...checkpoint.limitations ?? [], ...cast.limitations ?? []])];
	checkpoint.analyzedCastBatchCount = doneBatches + 1;
	if (checkpoint.analyzedCastBatchCount >= total || cast.done) {
		const countWarning = briefCountWarning(data.userBrief?.expectedCount ?? "", (checkpoint.characters ?? []).length);
	...
	} else console.info("[CHARACTERS] Batch persisted", {
...
async function runPipelineSlice(args) {
	...
	switch (args.phase) {
		...
		case "transcript":
			if (!checkpoint.transcript && !checkpoint.transcriptNote) try {
		...
		case "cast": {
			if (!checkpoint.completed.includes("cast")) await runOneCastBatch(data, frames, checkpoint);
			const total = Math.max(1, listCastBatches(frames).length);
			const doneCount = checkpoint.analyzedCastBatchCount ?? 0;
			if (!checkpoint.completed.includes("cast")) return finish("cast", 3, {
		...
		case "style":
			...
			if (!checkpoint.castValidated) {
				console.info("[CHARACTERS] Awaiting user validation", { count: checkpoint.characters?.length ?? 0 });
				return finish(longForm ? "segment" : "compact", 3, {
		...
		case "compact":
			...
			if (!checkpoint.visualStyle?.lockedStylePhrase) checkpoint.visualStyle = styleFromUserChoice(data.chosenStyleId, data.chosenStyleText);
			if (!checkpoint.analysis) checkpoint.analysis = await runCompactStep(data, frames, checkpoint);
			console.info("[PIPELINE] compact COMPLETE");
			if (!checkpoint.dialoguesValidated) return finish("produce", 6, {
				compact: true,
		...
		case "segment": {
			...
			if (checkpoint.completed.includes("segments")) return finish("narrative", 6, {
		...
		case "narrative":
			if (!checkpoint.analysis) checkpoint.analysis = await runNarrativeStep(data, frames, checkpoint);
			if (!checkpoint.dialoguesValidated) return finish("produce", 6, {
				analysis: checkpoint.analysis,
				awaitingDialogueReview: true,
				done: true
		...
		case "produce": {
			...
			if (!analysis) return finish("done", 7, {
				done: true,
				error: "Analyse absente pour générer les prompts."
			});
			...
			if (slice.error) return finish("done", 7, {
				done: true,
				error: slice.error,
				analysis
			});
			if (slice.done && slice.production) return finish("done", 7, {
				done: true,
				analysis,
...
//#endregion
export { runProductionSlice as a, runPipelineSlice as i, runReviseAnalysis as n, anatomyPromptBlock as o, runReviseProduction as r, analyze_core_exports as t };
