...
//#region node_modules/.nitro/vite/services/ssr/assets/store-CUNo2_1I.js
...
function frameTimes(duration) {
	...
	if (duration <= 12) return uniqueTimes([
		.12,
		duration * .28,
		duration * .55,
		duration * .82,
	...
	const hook = [.12, Math.min(1.6, duration * .06)];
	const restCount = Math.max(3, display - hook.length);
	const start = Math.min(2.4, duration * .1);
	const end = Math.max(start + .3, duration - .18);
	const rest = [];
	for (let i = 0; i < restCount; i += 1) rest.push(start + (end - start) * i / Math.max(1, restCount - 1));
...
//#endregion
export { cn as a, formatTimecode as c, useKreia as d, videoMetaFromElement as f, capturePlan as i, loadVideoElement as l, Button as n, extractFrames as o, Mark as r, formatDuration as s, AppShell as t, toAnalysisFrames as u };
